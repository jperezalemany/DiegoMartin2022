...
rule samtools_view_filter:
    ...
    log:
        "logs/samtools_view/{condition}_{sample}.clean.log"
    conda:
        "../envs/samtools.yml"
    threads: 4
    shell:
...
rule samtools_index_clean:
    ...
    output:
        "results/bamfiles/clean/single/{condition}/{sample}.clean.bam.bai"
    log:
        "logs/samtools_index/{condition}_{sample}.clean.log"
    conda:
        "../envs/samtools.yml"
    threads: 4
    shell:
...
rule picard_MergeSamFiles:
    input:
        lambda wildcards: expand(
            "results/clean_bam/{chip_sample}.clean.bam",
